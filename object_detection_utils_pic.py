...
class ObjectDetectionUtils:
    ...
    def visualize(
        ...
        """
        Visualize detections with proper coordinate handling
        """
        try:
            print("\n=== Debug: Visualization Process ===")
            ...
            print(f"Boxes shape: {boxes.shape if isinstance(boxes, np.ndarray) else 'No boxes'}")
            ...
            print(f"Image dimensions for drawing: {img_width}x{img_height}")
            ...
            for i in range(num_detections):
                try:
                    ...
                    # Calculate pixel coordinates directly from normalized coordinates
                    x1 = int(box[0] * img_width)
                    y1 = int(box[1] * img_height)
                    x2 = int(box[2] * img_width)
                    y2 = int(box[3] * img_height)
                    ...
                    print(f"Drawing detection {i}:")
                    print(f"- Class: {self.labels[classes[i]]}")
                    ...
                    print(f"Error drawing detection {i}: {det_error}")
                    continue
            
            # Save output
            output_file = os.path.join(output_path, f'output_image{image_id}.jpg')
            draw_image.save(output_file, 'JPEG')
            ...
            try:
                ...
                if isinstance(image, np.ndarray):
                    ...
                    if image.dtype != np.uint8:
                        image = (image * 255).astype(np.uint8)
                    image = Image.fromarray(image, 'RGB')
                output_file = os.path.join(output_path, f'output_image{image_id}_error.jpg')
                image.save(output_file, 'JPEG')
                print(f"Saved original image to: {output_file}")
            except Exception as save_error:
                print(f"Error saving original image: {save_error}")
                traceback.print_exc()